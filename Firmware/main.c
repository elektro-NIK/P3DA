...
ISR (TIMER0_OVF_vect) {
    if (!countPWM) {
        ...
        if (countPWM != leds_buff[RED][0])   PORT_RED0   &= ~(1<<RED0);
        if (countPWM != leds_buff[RED][1])   PORT_RED1   &= ~(1<<RED1);
        if (countPWM != leds_buff[RED][2])   PORT_RED2   &= ~(1<<RED2);
        if (countPWM != leds_buff[RED][3])   PORT_RED3   &= ~(1<<RED3);
        if (countPWM != leds_buff[RED][4])   PORT_RED4   &= ~(1<<RED4);
        if (countPWM != leds_buff[RED][5])   PORT_RED5   &= ~(1<<RED5);
        if (countPWM != leds_buff[GREEN][0]) PORT_GREEN0 &= ~(1<<GREEN0);
        if (countPWM != leds_buff[GREEN][1]) PORT_GREEN1 &= ~(1<<GREEN1);
        if (countPWM != leds_buff[GREEN][2]) PORT_GREEN2 &= ~(1<<GREEN2);
        if (countPWM != leds_buff[GREEN][3]) PORT_GREEN3 &= ~(1<<GREEN3);
        if (countPWM != leds_buff[GREEN][4]) PORT_GREEN4 &= ~(1<<GREEN4);
        if (countPWM != leds_buff[GREEN][5]) PORT_GREEN5 &= ~(1<<GREEN5);
        if (countPWM != leds_buff[BLUE][0])  PORT_BLUE0  &= ~(1<<BLUE0);
        if (countPWM != leds_buff[BLUE][1])  PORT_BLUE1  &= ~(1<<BLUE1);
        if (countPWM != leds_buff[BLUE][2])  PORT_BLUE2  &= ~(1<<BLUE2);
        if (countPWM != leds_buff[BLUE][3])  PORT_BLUE3  &= ~(1<<BLUE3);
        if (countPWM != leds_buff[BLUE][4])  PORT_BLUE4  &= ~(1<<BLUE4);
        if (countPWM != leds_buff[BLUE][5])  PORT_BLUE5  &= ~(1<<BLUE5);
    }
    if (countPWM == leds_buff[RED][0])   PORT_RED0   |= 1<<RED0;
    if (countPWM == leds_buff[RED][1])   PORT_RED1   |= 1<<RED1;
    if (countPWM == leds_buff[RED][2])   PORT_RED2   |= 1<<RED2;
